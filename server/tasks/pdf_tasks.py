        ...
        try:
            ...
            if "subject" not in base_cfg:
                ...
                if detected_subject and detected_subject != "Unknown":
                    if existing_subject != "unknown" and \
                            detected_subject.strip().lower() != existing_subject:
                        raise NonRetryableError(
                            f"Subject mismatch. Chat subject='{base_cfg.get('subject')}' "
                            f"but uploaded document subject='{detected_subject}'"
                        )
